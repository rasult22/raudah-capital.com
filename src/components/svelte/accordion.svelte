  ...
  <div class="relative mb-3 bg-[#edf7ff] rounded-[8px] font-railway">
    <h6 class="mb-0">
      <button
        ...
        class="relative text-[19px] flex items-center justify-between w-full p-4 text-left transition-all ease-in border-b border-solid cursor-pointer border-slate-100 text-slate-700 rounded-t-1 group text-dark-500"
    ...
    <div
      ...
      class={`${item.isOpen ? 'h-[145px] overflow-visible' : 'h-0  overflow-hidden'}  transition-all duration-300 ease-in-out`}
    >
      <div class="p-4 text-sm text-[15px] leading-normal text-blue-gray-500/80">
       {item.content}
      </div>
    </div>
  </div>
